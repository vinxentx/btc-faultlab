...
class PersistentRPCClient:
    ...
    def get_connection(self, timeout: int) -> HTTPConnection:
        if self._conn is None:
            self._conn = HTTPConnection(self.host, timeout=timeout)
        return self._conn
...
def main() -> int:
    ...
    def fetch_address() -> str:
        return rpc_call_with_retry(
            client, "getnewaddress", [], wallet=args.wallet, timeout=timeout
        )
